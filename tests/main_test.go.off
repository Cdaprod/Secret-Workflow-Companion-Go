...
// MockGHM is a mock implementation of the GHM interface (if needed)
type MockGHM struct {
	mock.Mock
}
...
func (m *MockGHM) AddSecretsToRepo(ctx context.Context, targetRepo string, secretNames []string, reposConfig *ReposConfig) error {
	args := m.Called(ctx, targetRepo, secretNames, reposConfig)
	return args.Error(0)
}

func (m *MockGHM) AddWorkflowsToRepo(ctx context.Context, targetRepo string, workflowNames []string, reposConfig *ReposConfig) error {
	args := m.Called(ctx, targetRepo, workflowNames, reposConfig)
	return args.Error(0)
}
...
func TestAddWorkflowSuccess(t *testing.T) {
	// Define test parameters
	repo := "owner/test-repo"
	workflowName := "ci.yml"
	workflowContent := `
...
// TestAddSecretStrategy_Execute tests the Execute method of AddSecretStrategy
func TestAddSecretStrategy_Execute(t *testing.T) {
	...
	strategy := &main.AddSecretStrategy{
		Token:       "test_token",
		Repo:        "owner/repo",
...
// TestStoreConfigStrategy_Execute tests the Execute method of StoreConfigStrategy
...
// TestLoadAndSaveReposConfig tests loading and saving ReposConfig
func TestLoadAndSaveReposConfig(t *testing.T) {
	...
	reposConfig := &main.ReposConfig{
		Repositories: map[string]main.RepoConfig{
			"owner/repo": {
	...
	err = main.SaveReposConfig(reposConfig, logger)
	assert.NoError(t, err)

	// Load reposConfig from tempFile
	loadedConfig, err := main.LoadReposConfig(logger)
	assert.NoError(t, err)
	assert.Equal(t, reposConfig, loadedConfig)
}

// TestAddSecretWithMock tests the add-secret command using a mock GitHub server
func TestAddSecretWithMock(t *testing.T) {
