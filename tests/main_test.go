...
import (
    ...
    // Replace "github.com/Cdaprod/secret-workflow-companion-go" with your actual module path
    mainpkg "github.com/Cdaprod/secret-workflow-companion-go"
...
func TestAddSecretStrategy_Execute(t *testing.T) {
    ...
    strategy := &mainpkg.AddSecretStrategy{
        Token:       "test_token",
        Repo:        "owner/repo",
...
func TestStoreConfigStrategy_Execute(t *testing.T) {
    ...
    strategy := &mainpkg.StoreConfigStrategy{
...
func TestLoadAndSaveReposConfig(t *testing.T) {
    ...
    reposConfig := &mainpkg.ReposConfig{
        Repositories: map[string]mainpkg.RepoConfig{
            "owner/repo": {
    ...
    err = mainpkg.SaveReposConfig(reposConfig, logger)
    assert.NoError(t, err)

    // Load reposConfig from tempFile
    loadedConfig, err := mainpkg.LoadReposConfig(logger)
    assert.NoError(t, err)
